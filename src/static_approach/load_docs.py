...
if __name__ == "__main__":
    ...
    df = load_doc_embeddings()
    df_text = load_doc_data()
    # df_text = df_text.sample(frac=1)
    # print(f'Label of row: {df_text.iloc[0].label}')
    print(df_text)
    print(df)

    # labels = df_text['label'][:10]
    # for label in labels1k:
    #     print (label)
    # print(tokenize(list(df_text.text)[0]))

    # index = build_inverted_index(df_text)
    # print(index['cat'])
    # print(index.keys())
    # print(len(index['a'])) 